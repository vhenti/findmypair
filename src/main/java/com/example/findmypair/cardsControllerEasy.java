...
public class cardsControllerEasy implements Initializable {
    ...
    MemoryGameEasyMode memoryGameEasyMode = new MemoryGameEasyMode();
    ...
    public cardsControllerEasy() throws FileNotFoundException {
    }

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        memoryGameEasyMode.setupGame(); // set up munaaa
    ...
    void buttonClicked(ActionEvent event) throws IOException {
        ...
        if(!firstButtonClicked){
            ...
            Color color = memoryGameEasyMode.getOptionAtIndex(firstButtonIndex);
        ...
        // change clicked button color
        Color color = memoryGameEasyMode.getOptionAtIndex(secondButtonIndex);
        BackgroundFill backgroundFill = new BackgroundFill(color, CornerRadii.EMPTY, Insets.EMPTY);
        Background background = new Background(backgroundFill);
        ...
        if(memoryGameEasyMode.checkTwoPositions(firstButtonIndex,secondButtonIndex)){
            score += 50;
            updateLabels();
            match = true;
            matchedButtons += 2;
            ...
            // it will load a new scene if na-match na lahat
            if (matchedButtons == 8) {
                loadScoreWindow();
            }
    ...
    private void saveScores() {
        try {
            FileWriter fileWriter = new FileWriter("C:\\Users\\jaira\\IdeaProjects\\findMyPair\\src\\main\\java\\scoresTextFiles\\easyScores.txt", true);
    ...
    private void showButtons(){
        for (int i = 0; i < 8; i++) {
            Color color = memoryGameEasyMode.getOptionAtIndex(i);
            BackgroundFill backgroundFill = new BackgroundFill(color, CornerRadii.EMPTY, Insets.EMPTY);
            Background background = new Background(backgroundFill);
    ...
    // kapag clinick mawawala yung styles tas pwede na lumabas yung colors
    //hindi na lalagyan ng question mark kasi nagdedelay ang mag fliflip na
    public void questionMarkHide(){
        buttons.get(firstButtonIndex).setStyle(null);
        buttons.get(secondButtonIndex).setStyle(null);
    }

    public void cardSEffects() {
        Media sound = new Media(new File("C:\\Users\\jaira\\IdeaProjects\\findMyPair\\src\\sounds\\bubble.mp3").toURI().toString());
    ...
    public void matchSEffects() {
        Media sound = new Media(new File("C:\\Users\\jaira\\IdeaProjects\\findMyPair\\src\\sounds\\twinkle.mp3").toURI().toString());
        MediaPlayer mediaPlayer = new MediaPlayer(sound);
    ...
    public void flipSEffects() {
        Media sound = new Media(new File("C:\\Users\\jaira\\IdeaProjects\\findMyPair\\src\\sounds\\flip.mp3").toURI().toString());
        MediaPlayer mediaPlayer = new MediaPlayer(sound);
    ...
    public void scoreSEffects() {
        Media sound = new Media(new File("C:\\Users\\jaira\\IdeaProjects\\findMyPair\\src\\sounds\\score.mp3").toURI().toString());
        MediaPlayer mediaPlayer = new MediaPlayer(sound);
    ...
    public void buttonSEffects() {
        Media sound = new Media(new File("C:\\Users\\jaira\\IdeaProjects\\findMyPair\\src\\sounds\\buttons.mp3").toURI().toString());
        MediaPlayer mediaPlayer = new MediaPlayer(sound);
